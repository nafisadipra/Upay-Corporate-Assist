...
def create_app(config_class=Config):
    ...
    weak_signing_secret = any(
        ...
        or any(
            ...
            for marker in ("replace", "change-in", "example", "development-secret")
        )
        for secret in signing_secrets
    )
    if (
        not app.config.get("TESTING")
        and app.config.get("FLASK_ENV") != "development"
        and weak_signing_secret
    ):
        raise RuntimeError(
            "SECRET_KEY and JWT_SECRET_KEY must each be random secrets of at least 32 bytes."
        )
