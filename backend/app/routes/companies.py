...
def create_company_wallet(company_id):
    ...
    if opening_balance != 0:
        return jsonify({'error': 'New wallets must start at zero. Allocate existing funds after creating the wallet.'}), 400
    ...
    if wallet_type == 'MAIN' and CentralWallet.query.filter_by(company_id=company_id, wallet_type='MAIN').first():
        return jsonify({'error': 'This corporate client already has a main disbursement wallet'}), 409
...
def transfer_company_wallet_funds(company_id):
    ...
    if not company:
        return jsonify({'error': 'Company not found'}), 404
    if company.status != 'ACTIVE':
        return jsonify({'error': 'Inactive or suspended companies cannot transfer wallet funds'}), 403
    ...
    try:
        ...
        return jsonify({'error': 'Source wallet, destination wallet, and a valid amount are required'}), 400
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'Wallet transfer could not be completed. No funds were moved.'}), 500
...
def update_company_wallet(company_id, wallet_id):
    ...
    if 'balance' in data:
        return jsonify({'error': 'Direct balance editing is disabled. Use an audited top-up or wallet transfer.'}), 400
