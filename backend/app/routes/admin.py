...
def get_overview():
    ...
    wallet_total = sum(
        (
            ...
            for wallet in CentralWallet.query.filter_by(status="ACTIVE").all()
        ),
        Decimal("0.00"),
    )
...
def get_company_employees(company_id):
    ...
    employees = (
        Employee.query.filter_by(company_id=company_id).order_by(Employee.employee_code.asc()).all()
    )
    return (
        jsonify(
            {
...
def upload_employee_roster(company_id):
    ...
    if "file" not in request.files or request.files["file"].filename == "":
        return (
            jsonify({"error": "Please attach an Excel (.xlsx/.xls) or CSV employee roster file."}),
            400,
        )
    ...
    file_path = os.path.join(
        current_app.config["UPLOAD_FOLDER"], f"roster_{company_id}_{uuid4().hex}_{filename}"
    )
...
def add_single_employee(company_id):
    ...
    if existing:
        return (
            jsonify(
                {"error": f"Employee with phone number {phone} is already on this company roster"}
            ),
            409,
        )
    ...
    emp = Employee(
        company_id=company_id,
        employee_code=code
        or f"EMP-{int(Employee.query.filter_by(company_id=company_id).count()) + 1001}",
        phone_number=phone,
        employee_name=name,
        department=dept,
    ...
    return (
        jsonify(
            {"message": "Employee added to approved corporate roster", "employee": emp.to_dict()}
        ),
        201,
    )
...
def get_company_bank_accounts(company_id):
    ...
    accounts = (
        CompanyBankAccount.query.filter_by(company_id=company_id)
...
def add_company_bank_account(company_id):
    ...
    if is_primary:
        CompanyBankAccount.query.filter_by(company_id=company_id, is_primary=True).update(
            {"is_primary": False}
        )
    ...
    bank_acc = CompanyBankAccount(
        ...
        account_type=(
            account_type if account_type in ["CURRENT", "SETTLEMENT", "ESCROW"] else "CURRENT"
        ),
        status="ACTIVE",
...
def create_user():
    ...
    if not all([full_name, email, phone_number, password]) or role not in {
        "MAKER",
        "CHECKER",
        "ADMIN",
    }:
        return (
            jsonify({"error": "Name, email, phone, password, and a valid role are required"}),
            400,
        )
    if len(password) < 12:
    ...
    if role != "ADMIN" and not Company.query.get(company_id):
        return (
            jsonify({"error": "A valid corporate client is required for maker and checker users"}),
            400,
        )
...
def get_activity():
    ...
    alerts = (
        RiskAlert.query.filter_by(review_status="PENDING_REVIEW")
