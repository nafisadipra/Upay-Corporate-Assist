...
from app.services.disbursement_service import (
    canonical_phone,
    record_checker_review,
    execute_batch_disbursement,
)
...
def ensure_active_company(company_id):
    ...
    if company.status != "ACTIVE":
        return None, (
            jsonify(
                {
                    "error": "This corporate client is inactive or suspended. Payroll actions are blocked."
                }
            ),
            403,
        )
...
def upload_batch():
    ...
    if "file" in request.files and request.files["file"].filename != "":
        ...
        if file and allowed_file(file.filename):
            ...
            file_path = os.path.join(
                current_app.config["UPLOAD_FOLDER"], f"payroll_{uuid4().hex}_{filename}"
            )
            file_name = filename
            try:
                file.save(file_path)
            ...
            return (
                jsonify({"error": "Invalid file format. Allowed formats: .xlsx, .xls, .csv"}),
                400,
            )
    else:
    ...
    if user.role == "ADMIN":
        if requested_company_id is None:
            return (
                jsonify({"error": "company_id is required when an administrator uploads a batch."}),
                400,
            )
        try:
        ...
        if user.company_id is None:
            return (
                jsonify(
                    {
                        "error": "Corporate users must be assigned to a company before uploading batches."
                    }
                ),
                403,
            )
        if requested_company_id is not None:
            ...
            if requested_company_id != user.company_id:
                return (
                    jsonify(
                        {
                            "error": "Tenant boundary violation: Cannot upload a batch for another corporate client."
                        }
                    ),
                    403,
                )
    ...
    duplicate_phones = sorted(
        phone for phone, count in Counter(phones).items() if phone and count > 1
    )
    if duplicate_phones:
        return (
            jsonify(
    ...
    for row in parsed_rows:
        ...
        if basic_salary < 0 or gross_salary < 0 or gross_salary < basic_salary:
            return (
                jsonify({"error": "Gross salary must be greater than or equal to basic salary."}),
                400,
            )
    ...
    flagged_anomalies = sum(
        1 for item in batch_items if item.is_anomaly or item.account_validation_status != "VALID"
    )
...
def get_batch(batch_id):
    ...
    if user.role != "ADMIN" and batch.company_id != user.company_id:
        return (
            jsonify(
                {"error": "Tenant isolation: Access to other corporate client batch is forbidden"}
            ),
            403,
        )
...
def get_batch_items(batch_id):
    ...
    if user.role != "ADMIN" and batch.company_id != user.company_id:
        return (
            jsonify(
                {
                    "error": "Tenant isolation: Access to other corporate client batch items is forbidden"
                }
            ),
            403,
        )
...
def download_batch_workbook(batch_id):
    ...
    for item in BatchItem.query.filter_by(batch_id=batch.id).order_by(BatchItem.id.asc()).all():
        sheet.append(
            [
                item.employee_name,
                item.corrected_phone_number or item.raw_phone_number,
                item.department or "",
...
def download_executed_batch_archive(batch_id):
    ...
    if batch.status != "EXECUTED":
        return (
            jsonify({"error": "A payroll archive is available only after final disbursement."}),
            409,
        )
    ...
    return send_file(
        output,
        as_attachment=True,
        download_name=f"payroll-archive-{period}-batch-{batch.id}.xlsx",
        mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )
...
def correct_item_phone(item_id):
    ...
    if g.current_user.role != "ADMIN" and batch.company_id != g.current_user.company_id:
        return (
            jsonify({"error": "Tenant isolation: Cannot edit item of another corporate client"}),
            403,
        )
    ...
    new_phone = str(
        data.get("corrected_phone_number", item.corrected_phone_number or item.raw_phone_number)
    ).strip()
    ...
    if any(
        canonical_phone(other.corrected_phone_number or other.raw_phone_number)
        == new_canonical_phone
        for other in sibling_items
    ):
    ...
    batch.valid_records = sum(
        1 for i in items if i.account_validation_status in ["VALID", "UNRECOGNIZED_PAYEE"]
    )
    batch.invalid_records = sum(
        1 for i in items if i.account_validation_status not in ["VALID", "UNRECOGNIZED_PAYEE"]
    )
    batch.flagged_anomalies = sum(
        1 for i in items if i.is_anomaly or i.account_validation_status != "VALID"
    )
...
def submit_batch(batch_id):
    ...
    if batch.status not in ["DRAFT", "VALIDATED", "FLAGGED_RISK", "RETURNED_TO_HR"]:
        return (
            jsonify({"error": f"Batch cannot be submitted from current status {batch.status}"}),
            400,
        )

    if BatchItem.query.filter_by(batch_id=batch.id, item_status="REJECTED").count():
        return (
            jsonify({"error": "Correct every item returned by Finance before resubmitting."}),
            400,
        )
...
def checker_review(batch_id):
    ...
    if g.current_user.role != "ADMIN" and batch.company_id != g.current_user.company_id:
        return (
            jsonify({"error": "Tenant isolation: Cannot review other corporate client batch"}),
            403,
        )
    ...
    batch = Batch.query.get(batch_id)
    return jsonify({"message": msg, "batch": batch.to_dict()}), 200
