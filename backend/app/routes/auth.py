...
from app.utils.portal_session import ADMIN_PORTAL, auth_cookie_name, current_portal
...
def login():
    ...
    if user.status != "ACTIVE":
        return (
            jsonify({"error": "Your user account is suspended or inactive. Contact support."}),
            403,
        )
    ...
    if portal == ADMIN_PORTAL and user.role != "ADMIN":
        return jsonify({"error": "This workspace is available to upay administrators only."}), 403
    if portal != ADMIN_PORTAL and user.role == "ADMIN":
        return jsonify({"error": "Administrator accounts must use the upay Admin portal."}), 403
...
def get_session():
    ...
    bearer_token = (
        auth_header.split(" ", 1)[1].strip() if auth_header.startswith("Bearer ") else None
    )
