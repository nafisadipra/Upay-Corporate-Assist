...
def upload_registrations():
    ...
    if not company or company.status != "ACTIVE":
        return (
            jsonify(
                {
                    "error": "Your corporate client is inactive or suspended. Employee registrations are blocked."
                }
            ),
            403,
        )
    ...
    if (
        not filename
        or "." not in filename
        or filename.rsplit(".", 1)[1].lower() not in current_app.config["ALLOWED_EXTENSIONS"]
    ):
        return jsonify({"error": "Attach a valid .xlsx, .xls, or .csv registration template."}), 400
    os.makedirs(current_app.config["UPLOAD_FOLDER"], exist_ok=True)
    path = os.path.join(
        current_app.config["UPLOAD_FOLDER"],
        f"registration_{g.current_user.id}_{uuid4().hex}_{filename}",
    )
    file.save(path)
    try:
        ...
        if os.path.exists(path):
            os.remove(path)
    if not rows:
        return jsonify({"error": "No valid registration rows found."}), 400
    for row in rows:
        existing = EmployeeRegistration.query.filter_by(
        ...
        if existing:
            ...
            db.session.add(
                EmployeeRegistration(
                    company_id=g.current_user.company_id, submitted_by=g.current_user.id, **row
                )
            )
    db.session.add(
        AuditLog(
            ...
            details={"file_name": filename, "rows": len(rows)},
        )
    )
    db.session.commit()
    return (
        jsonify(
            {
                "message": f"{len(rows)} employee registrations sent to Upay Admin for approval.",
                "rows": len(rows),
            }
        ),
        201,
...
def approve_registration(registration_id):
    ...
    if not row or row.status != "PENDING_ADMIN_APPROVAL":
    ...
    if not company or company.status != "ACTIVE":
        return (
            jsonify(
                {
                    "error": "This corporate client is inactive or suspended. Registration approval is blocked."
                }
            ),
            403,
        )
    employee = Employee.query.filter_by(
        company_id=row.company_id, phone_number=row.wallet_details
    ).first()
    if employee is None:
        employee = Employee(
            company_id=row.company_id,
            employee_code=row.employee_code,
            phone_number=row.wallet_details,
        ...
        (
            ...
            employee.status,
        ) = (row.employee_code, row.full_name, row.department, row.designation, "ACTIVE")
    if not Account.query.filter_by(phone_number=row.wallet_details).first():
        db.session.add(
            Account(
                phone_number=row.wallet_details,
                account_holder_name=row.full_name,
    ...
    db.session.add(
        AuditLog(
            ...
            details={
                ...
                "email": row.email,
                "wallet_details": row.wallet_details,
            },
        )
    )
    db.session.commit()
    return jsonify(
        {
            "message": "Employee registered in the Upay roster and account database.",
...
def bulk_approve_registrations():
    ...
    if (
        ...
        or not all(isinstance(item, int) for item in registration_ids)
    ):
    ...
    if len(rows) != len(registration_ids) or any(
        row.status != "PENDING_ADMIN_APPROVAL" for row in rows
    ):
    ...
    if any(not company or company.status != "ACTIVE" for company in companies.values()):
        return (
            jsonify(
                {
                    "error": "A selected corporate client is inactive or suspended. Registration approval is blocked."
                }
            ),
            403,
        )
    ...
    for row in rows:
        ...
        row.status, row.reviewed_by, row.reviewed_at = (
            "APPROVED",
            g.current_user.id,
            datetime.utcnow(),
        )
    ...
    return (
        jsonify(
            {
                "message": f"{len(rows)} employee registrations approved.",
