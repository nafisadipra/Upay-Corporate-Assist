...
def get_disbursement_history(company_id):
    ...
    for batch in Batch.query.filter_by(company_id=company_id, status="EXECUTED").all():
        period_date = batch.payroll_period or (
            batch.executed_at.date() if batch.executed_at else None
        )
        if not period_date:
            continue
        period = period_date.strftime("%Y-%m")
        monthly_totals[period] = monthly_totals.get(period, Decimal("0.00")) + Decimal(
...
def get_liquidity_forecast(company_id):
    ...
    include_bonus = (
        None
        if include_bonus_arg is None
        else include_bonus_arg.strip().lower() in {"1", "true", "yes"}
    )
    return (
        jsonify(
            get_cached_liquidity_forecast(
                company_id, horizon=horizon, include_festival_bonus=include_bonus
            )
        ),
        200,
    )
...
def refresh_liquidity_forecast_endpoint(company_id):
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "error": "Forecast refresh failed. The last successful forecast remains available."
                }
            ),
            500,
        )
...
def save_forecast_settings(company_id):
    try:
        settings = update_forecast_settings(
            company_id, request.get_json(silent=True) or {}, g.current_user.id
        )
...
def get_dashboard_summary(company_id):
    ...
    total_disbursed = sum(
        (Decimal(str(b.total_amount)) for b in batches if b.status == "EXECUTED"), Decimal("0.00")
    )
    pending_approval = sum(
        (
            Decimal(str(b.total_amount))
            for b in batches
            if b.status in ["FLAGGED_RISK", "PENDING_CHECKER_REVIEW", "CHECKER_REVIEWED"]
        ),
        Decimal("0.00"),
    )
    ...
    open_alerts_count = (
        ...
        .join(Batch)
        .filter(Batch.company_id == company_id, RiskAlert.review_status == "PENDING_REVIEW")
        .count()
    )
