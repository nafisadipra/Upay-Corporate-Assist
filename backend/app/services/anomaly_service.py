...
def evaluate_batch_items_anomalies(company_id, batch_items):
    ...
    for item in batch_items:
        ...
        if item.account_validation_status in [
            "INVALID_LENGTH",
            ...
            flag_type = (
                "UNREGISTERED_PHONE"
                if item.account_validation_status == "UNREGISTERED_ACCOUNT"
                else "ACCOUNT_INACTIVE"
            )
            severity = (
                "CRITICAL" if item.account_validation_status == "UNREGISTERED_ACCOUNT" else "HIGH"
            )
            ...
            create_risk_alert(
                item.id, flag_type=flag_type, severity=severity, notes=item.anomaly_reason
            )
            continue
        ...
        if item.account_validation_status == "UNRECOGNIZED_PAYEE":
            ...
            create_risk_alert(
                item.id, flag_type="ROSTER_MISMATCH", severity="CRITICAL", notes=item.anomaly_reason
            )
            continue
        ...
        employee_history = (
            PayrollHistory.query.filter_by(company_id=company_id, phone_number=phone)
            .order_by(PayrollHistory.disbursement_date.desc())
            .all()
        )
        ...
        if completed_cycles < 3:
            ...
            dept_history = PayrollHistory.query.filter_by(
                company_id=company_id, department=item.department
            ).all()
