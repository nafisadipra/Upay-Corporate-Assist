...
def record_checker_review(
    batch_id, checker_id, review_status="APPROVED_BY_CHECKER", review_notes=None
):
    """
    ...
    if batch.maker_id == checker_id:
        return (
            False,
            "Maker-Checker Segregation Violation: The Maker who uploaded this batch cannot perform the Checker review.",
        )

    if batch.status != "PENDING_CHECKER_REVIEW":
        return (
            False,
            f"Batch can only be reviewed from 'PENDING_CHECKER_REVIEW', not '{batch.status}'.",
        )

    open_alerts = (
        RiskAlert.query.join(BatchItem)
        .filter(
    ...
    if open_alerts:
        return (
            False,
            "Resolve every AI alert by overriding it or raising an issue before signing off the batch.",
        )
    ...
    batch.checker_notes = (
        review_notes or "Batch reviewed and signed off by Finance Director (Checker)."
    )
...
def execute_batch_disbursement(batch_id, executing_user_id):
    ...
    if batch.status not in ["CHECKER_REVIEWED", "PENDING_CHECKER_APPROVAL", "APPROVED"]:
        return (
            False,
            f"Batch cannot be executed from current status '{batch.status}'. Checker review sign-off is required.",
        )
    ...
    if not company or company.status != "ACTIVE":
        return (
            False,
            "This corporate client is inactive or suspended. Payroll execution is blocked.",
        )

    items = BatchItem.query.filter_by(batch_id=batch.id).all()
    phones = [
        canonical_phone(item.corrected_phone_number or item.raw_phone_number) for item in items
    ]
    if len(phones) != len(set(phones)):
        return (
            False,
            "Batch contains duplicate payout phone numbers. Each payee may appear only once.",
        )
    blocked_items = [
        item.id
        for item in items
        if item.account_validation_status != "VALID" or item.item_status == "REJECTED"
    ]
    if blocked_items:
        return (
            False,
            "Batch contains invalid or rejected payout items. Correct or remove them before execution.",
        )

    open_alerts = (
        RiskAlert.query.join(BatchItem)
        .filter(BatchItem.batch_id == batch.id, RiskAlert.review_status == "PENDING_REVIEW")
        .count()
    )
    if open_alerts:
    ...
    try:
        ...
        wallet = (
            CentralWallet.query.filter_by(company_id=batch.company_id, wallet_type="MAIN")
            .with_for_update()
            .first()
        )
        ...
        if wallet_balance < batch_amount:
            return (
                False,
                f"Insufficient central wallet balance. Required: {float(batch_amount):,.2f} BDT, Available: {float(wallet_balance):,.2f} BDT. Please contact Upay Admin to top up disbursement funds.",
            )
        ...
        now = datetime.utcnow()
        forecast_history_date = (
            datetime.combine(batch.payroll_period, datetime.min.time())
            if batch.payroll_period
            else now
        )
        for item in items:
        ...
        try:
            ...
            current_app.logger.exception(
                "Liquidity forecast refresh failed after batch %s execution.", batch.id
            )

        return (
            True,
            f"Disbursement executed successfully! BDT {float(batch_amount):,.2f} debited from central wallet.",
        )
