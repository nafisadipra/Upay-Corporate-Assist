...
def build_monthly_series(company_id: int) -> list[dict[str, Any]]:
    ...
    rows = (
        PayrollHistory.query.filter_by(company_id=company_id)
        .order_by(PayrollHistory.disbursement_date)
        .all()
    )
    for row in rows:
...
def _holt(values: list[float], horizon: int) -> tuple[list[float], list[float], dict[str, Any]]:
    ...
    for actual in values[1:]:
        ...
        trend = beta * (level - previous_level) + (1 - beta) * trend
    return (
        [level + trend * step for step in range(1, horizon + 1)],
        errors,
        {"engine": "native_grid", "alpha": alpha, "beta": beta},
    )
...
def _seasonal_fallback(
    ...
    forecast = [
        level + monthly_trend * step + seasonality[(len(values) + step - 1) % 12]
        for step in range(1, horizon + 1)
    ]
    residuals = [
        values[index] - (level + seasonality[index % 12])
        for index in range(len(values) - 12, len(values))
    ]
    return forecast, residuals, {"engine": "native_seasonal_fallback", "seasonal_periods": 12}


def _fit_values(
    values: list[float], strategy: str, horizon: int
) -> tuple[list[float], list[float], dict[str, Any]]:
    if strategy == "SES":
        if HAS_STATSMODELS:
            fit = ExponentialSmoothing(values, initialization_method="estimated").fit(
                optimized=True
            )
            return (
                list(fit.forecast(horizon)),
                list(np.asarray(values) - np.asarray(fit.fittedvalues)),
                {"engine": "statsmodels", "alpha": float(fit.params.get("smoothing_level", 0.0))},
            )
...
def _prediction_bounds(
    predictions: list[float], residuals: list[float]
) -> list[tuple[float, float]]:
    deviation = pstdev(residuals) if len(residuals) > 1 else 0.0
    bounds = []
    for amount in predictions:
        lower_bound = max(0.0, amount - 1.96 * deviation)
        upper_bound = max(0.0, amount + 1.96 * deviation)
...
def _base_forecast(company_id: int, horizon: int) -> dict[str, Any]:
    ...
    if strategy == "BASELINE":
        predictions = [baseline] * horizon
        residuals = []
        parameters = {"engine": "configured_baseline"}
        assumptions = [
            "Configured planning baseline; insufficient payroll history for a trained model."
        ]
        mae = None
        mape = None
        folds = 0
        ...
        if strategy != "HOLT_WINTERS_ADDITIVE":
            assumptions.append(
                "Yearly festival seasonality is not learned until 24 completed monthly observations."
            )
    ...
    for index, (amount, (lower, upper)) in enumerate(zip(predictions, bounds), start=1):
        records.append(
            {
                "period": _period(_add_months(latest_month, index)),
                "predicted_amount": round(float(max(0.0, amount)), 2),
...
def _create_accuracy_alert(
    company_id: int, run: ForecastRun, mape: float | None, folds: int
) -> None:
    if mape is None or folds < 6 or mape <= 15:
        return
    existing = ForecastAlert.query.filter_by(
        company_id=company_id,
        alert_type="HIGH_FORECAST_ERROR",
    ...
    if existing is None:
        alert = ForecastAlert(
            ...
            alert_type="HIGH_FORECAST_ERROR",
            severity="MEDIUM",
            details={"mape": mape, "backtest_folds": folds, "threshold": 15},
        )
        db.session.add(alert)


def _create_forecast_run(company_id: int, result: dict[str, Any]) -> ForecastRun:
    parameters = {
        **result["parameters"],
    ...
    return ForecastRun(
        ...
        mape=result["mape"],
        parameters=parameters,
        source_data_through=result["source_data_through"],
        completed_at=datetime.utcnow(),
    )


def _save_forecast_record(
    company_id: int,
    run: ForecastRun,
    result: dict[str, Any],
    record: dict[str, Any],
) -> None:
    row = LiquidityForecast.query.filter_by(
        company_id=company_id,
        forecast_period=record["period"],
    ).first()
    if row is None:
        row = LiquidityForecast(
            company_id=company_id,
            forecast_period=record["period"],
    ...
    row.predicted_amount = record["predicted_amount"]
    row.lower_bound = record["lower_bound"]
    row.upper_bound = record["upper_bound"]
    row.model_type = result["strategy"]
    row.status = result["status"]
    row.history_months = result["history_months"]
    row.mae = result["mae"]
    row.mape = result["mape"]
    row.confidence_score = CONFIDENCE_LEVEL
    row.assumptions = record["assumptions"]
    row.source_data_through = result["source_data_through"]
    row.forecast_run_id = run.id
    row.generated_at = datetime.utcnow()

    run_result = ForecastRunResult(
        forecast_run_id=run.id,
        forecast_period=record["period"],
        predicted_amount=record["predicted_amount"],
...
def refresh_liquidity_forecast(company_id: int, horizon: int = 3) -> dict[str, Any]:
    ...
    try:
        ...
        run = _create_forecast_run(company_id, result)
        db.session.add(run)
        db.session.flush()

        for record in result["records"]:
            _save_forecast_record(company_id, run, result, record)
...
def _apply_festival_adjustment(
    row: dict[str, Any], setting: CompanyForecastSettings | None, include_bonus: bool
) -> dict[str, Any]:
    adjusted = dict(row)
    assumptions = list(row.get("assumptions") or [])
    forecast_month = int(row["period"].split("-")[1])
    bonus_months = (
        {int(month) for month in (setting.festival_bonus_months or [])} if setting else set()
    )
    has_configured_bonus = setting is not None and float(setting.festival_bonus_amount) > 0

    if include_bonus and forecast_month in bonus_months and has_configured_bonus:
...
def _future_cached_rows(
    company_id: int, latest_completed_period: str, horizon: int
) -> list[LiquidityForecast]:
    """Return valid cached forecasts after the last completed payroll month."""
    all_rows = (
        LiquidityForecast.query.filter_by(company_id=company_id)
        .order_by(LiquidityForecast.forecast_period)
        .all()
    )
    future_rows = []
    for row in all_rows:
        is_month_period = len(row.forecast_period) == 7 and row.forecast_period[4] == "-"
        if is_month_period and row.forecast_period > latest_completed_period:
...
def _historical_series_payload(series: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [{"period": item["period"], "amount": item["amount"]} for item in series]


def _model_payload(row: LiquidityForecast) -> dict[str, Any]:
    return {
        "type": row.model_type,
        "status": row.status,
...
def _unavailable_forecast_payload(
    company_id: int,
    series: list[dict[str, Any]],
    setting: CompanyForecastSettings | None,
) -> dict[str, Any]:
    source_data_through = series[-1]["period"] if series else None
    return {
        "company_id": company_id,
        "generated_at": None,
        "source_data_through": source_data_through,
        ...
        "forecasts": [],
        "liquidity_forecasts": [],
        "historical_series": _historical_series_payload(series),
        "settings": setting.to_dict() if setting else None,
    }


def get_cached_liquidity_forecast(
    company_id: int,
    horizon: int = 3,
    include_festival_bonus: bool | None = None,
) -> dict[str, Any]:
    """Return cached forecasts and apply optional request-time festival bonuses."""
    series = build_monthly_series(company_id)
    setting = _settings(company_id)
    latest_completed_period = series[-1]["period"] if series else date.today().strftime("%Y-%m")
    rows = _future_cached_rows(company_id, latest_completed_period, horizon)

    if include_festival_bonus is None:
        include_bonus = setting.include_festival_bonus if setting else False
    else:
        include_bonus = bool(include_festival_bonus)
    ...
    if not rows:
        return _unavailable_forecast_payload(company_id, series, setting)
    ...
    for row in rows:
        ...
        payload["topup_required"] = round(
            max(0.0, payload["predicted_amount"] - current_balance), 2
        )
        forecasts.append(payload)

    generated_at = base.generated_at.isoformat() if base.generated_at else None
    source_data_through = base.source_data_through.isoformat() if base.source_data_through else None
    historical_series = _historical_series_payload(series)
    settings_payload = setting.to_dict() if setting else None
    return {
        "company_id": company_id,
        "generated_at": generated_at,
        "source_data_through": source_data_through,
        "model": _model_payload(base),
        "forecasts": forecasts,
        "liquidity_forecasts": forecasts,
...
def update_forecast_settings(
    company_id: int, payload: dict[str, Any], configured_by: int
) -> CompanyForecastSettings:
    setting = _settings(company_id)
    if setting is None:
    ...
    if not isinstance(months, list) or any(
        not isinstance(month, int) or month < 1 or month > 12 for month in months
    ):
        raise ValueError("Festival bonus months must be integers from 1 to 12.")
    setting.planning_baseline_amount = baseline
    setting.festival_bonus_amount = bonus_amount
    setting.festival_bonus_months = sorted(set(months))
    setting.include_festival_bonus = bool(
        payload.get("include_festival_bonus", setting.include_festival_bonus)
    )
