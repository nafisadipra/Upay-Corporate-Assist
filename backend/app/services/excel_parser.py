...
def parse_employee_registration_file(file_path):
    ...
    for _, row in df.iterrows():
        email, name, wallet = (str(row.get(key, '')).strip() for key in ('email', 'full_name', 'wallet_details'))
        ...
        if not wallet.isdigit() or len(wallet) != 11 or not wallet.startswith('01'):
            raise ValueError(f'Invalid Upay registration number for {email}. Use an 11-digit number beginning with 01.')
        rows.append({key: (None if pd.isna(row.get(key)) else str(row.get(key)).strip()) for key in ('email', 'full_name', 'employee_code', 'division', 'region', 'department', 'designation', 'employment_status', 'exit_date', 'wallet_details')})
...
def parse_payroll_file(file_path):
    ...
    # Map column aliases
    phone_col = next((c for c in df.columns if 'phone' in c or 'mobile' in c or 'number' in c or 'account' in c), 'phone_number')
    ...
    for idx, row in df.iterrows():
        ...
        department = str(row.get(dept_col, "General")).strip() if pd.notna(row.get(dept_col)) else "General"
...
def parse_employee_roster_file(file_path):
    ...
    code_col = next((c for c in df.columns if 'code' in c or 'id' in c or 'emp_id' in c or 'employee_id' in c), 'employee_code')
    name_col = next((c for c in df.columns if 'name' in c or 'employee' in c), 'employee_name')
    phone_col = next((c for c in df.columns if 'phone' in c or 'mobile' in c or 'upay' in c or 'account' in c or 'number' in c), 'phone_number')
    dept_col = next((c for c in df.columns if 'dept' in c or 'department' in c), 'department')
    desig_col = next((c for c in df.columns if 'desig' in c or 'title' in c or 'role' in c or 'position' in c), 'designation')
    ...
    for idx, row in df.iterrows():
        ...
        code = str(row.get(code_col, f"EMP-{idx+1001}")).strip() if pd.notna(row.get(code_col)) else f"EMP-{idx+1001}"
        name = str(row.get(name_col, f"Employee_{idx+1}")).strip()
        dept = str(row.get(dept_col, "General")).strip() if pd.notna(row.get(dept_col)) else "General"
        designation = str(row.get(desig_col, "Staff")).strip() if pd.notna(row.get(desig_col)) else "Staff"
