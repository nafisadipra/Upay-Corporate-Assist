...
from app.utils.portal_session import auth_cookie_name
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                return (
                    jsonify({"error": "Authentication token has expired. Please sign in again."}),
                    401,
                )
            ...
            if user.status != "ACTIVE":
                return (
                    jsonify({"error": "User account is inactive or suspended. Access denied."}),
                    403,
                )
        ...
        def decorated_function(*args, **kwargs):
            ...
            if req_company_id is not None and int(req_company_id) != user.company_id:
                return (
                    jsonify(
                        {
                            "error": "Tenant boundary violation: Access to other corporate client data is forbidden."
                        }
                    ),
                    403,
                )
