...
def test_admin_bank_accounts_add_and_remove(client, admin_auth):
    ...
    res_add = client.post(
        "/api/admin/companies/1/bank-accounts",
        json={
...
def test_admin_single_employee_add_and_remove(client, admin_auth):
    # Add employee
    res_add = client.post(
        "/api/admin/companies/1/employees",
        json={
...
def test_admin_bulk_approve_employee_registrations(client, app, admin_auth):
    with app.app_context():
        rows = [
            EmployeeRegistration(
                company_id=1,
                submitted_by=1,
                full_name="Bulk Employee One",
                email="bulk-one@pran.com",
                employee_code="EMP-BULK-01",
                department="Finance",
                designation="Officer",
                wallet_details="01717770001",
            ),
            EmployeeRegistration(
                company_id=1,
                submitted_by=1,
                full_name="Bulk Employee Two",
                email="bulk-two@pran.com",
                employee_code="EMP-BULK-02",
                department="Sales",
                designation="Executive",
                wallet_details="01717770002",
            ),
        ]
    ...
    response = client.post(
        "/api/employee-registrations/bulk-approve",
    ...
    with app.app_context():
        approved_rows = EmployeeRegistration.query.filter(
            EmployeeRegistration.id.in_(registration_ids)
        ).all()
