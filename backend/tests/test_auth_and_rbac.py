...
def test_login_success(client):
    res = client.post(
        "/api/auth/login", json={"email": "maker1@pran.com", "password": "password123"}
    )
...
def test_corporate_and_admin_cookie_sessions_are_independent(client):
    corporate = {"X-Upay-Portal": "corporate"}
    admin = {"X-Upay-Portal": "admin"}

    corporate_login = client.post(
        "/api/auth/login",
        headers=corporate,
        json={
    ...
    assert corporate_login.status_code == 200
    assert "upay_test_session=" in corporate_login.headers["Set-Cookie"]
    ...
    assert "upay_admin_session=" in admin_login.headers["Set-Cookie"]

    assert client.get("/api/auth/session", headers=corporate).get_json()["user"]["role"] == "MAKER"
    assert client.get("/api/auth/session", headers=admin).get_json()["user"]["role"] == "ADMIN"
...
def test_login_invalid_password(client):
    res = client.post(
        "/api/auth/login", json={"email": "maker1@pran.com", "password": "wrongpassword"}
    )
...
def test_login_inactive_user_blocked(client):
    res = client.post(
        "/api/auth/login", json={"email": "inactive@pran.com", "password": "password123"}
    )
    assert res.status_code == 403
    assert (
        "suspended" in res.get_json()["error"].lower()
        or "inactive" in res.get_json()["error"].lower()
    )
