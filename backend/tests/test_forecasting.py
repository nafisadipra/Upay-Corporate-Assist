...
def add_company_one_monthly_history():
    for month, amount in enumerate(
        [4_100_000, 4_200_000, 4_350_000, 4_300_000, 4_500_000, 4_650_000], start=1
    ):
        db.session.add(
            PayrollHistory(
                company_id=1,
...
def test_cached_forecast_read_does_not_create_another_model_run(client, app, maker_auth):
    with app.app_context():
        add_company_one_monthly_history()
    assert (
        client.post("/api/analytics/liquidity-forecast/1/refresh", headers=maker_auth).status_code
        == 200
    )
    with app.app_context():
...
def test_company_maker_configures_a_real_festival_bonus_adjustment(client, app, maker_auth):
    ...
    base = client.get(
        "/api/analytics/liquidity-forecast/1?include_festival_bonus=false", headers=maker_auth
    ).get_json()["forecasts"][0]
    forecast_month = int(base["period"].split("-")[1])
    settings = client.put(
        "/api/analytics/liquidity-forecast/1/settings",
        headers=maker_auth,
        json={
    ...
    adjusted = client.get(
        "/api/analytics/liquidity-forecast/1?include_festival_bonus=true", headers=maker_auth
    ).get_json()["forecasts"][0]
...
def test_executed_payroll_refreshes_the_company_forecast(client, app, maker_auth, checker_auth):
    ...
    assert after_execution.get_json()["historical_series"] == [
        {"period": "2026-04", "amount": 45000.0}
    ]
    ...
    with app.app_context():
        ...
        assert (
            PayrollHistory.query.filter_by(company_id=1).one().disbursement_date.date().isoformat()
            == "2026-04-01"
        )
...
def test_upload_rejects_an_invalid_payroll_month_and_allows_future_months(client, maker_auth):
    ...
    invalid = client.post(
        "/api/batches/upload",
        headers=maker_auth,
        json={"payroll_period": "April 2026", "items": [salary_row]},
    )
    future = client.post(
        "/api/batches/upload",
        headers=maker_auth,
        json={"payroll_period": "2099-01", "items": [salary_row]},
    )
    assert invalid.status_code == 400
    assert future.status_code == 201
    assert future.get_json()["batch"]["payroll_period"] == "2099-01-01"
    batch_id = future.get_json()["batch"]["id"]
    audit_logs = client.get(f"/api/audit-logs?batch_id={batch_id}", headers=maker_auth).get_json()[
        "audit_logs"
    ]
    assert audit_logs
...
def test_upay_admin_cannot_read_or_refresh_a_company_forecast(client, admin_auth):
    ...
    assert (
        client.post("/api/analytics/liquidity-forecast/1/refresh", headers=admin_auth).status_code
        == 403
    )
