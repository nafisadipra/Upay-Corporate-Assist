...
def test_maker_transfers_existing_money_between_company_wallets(client, maker_auth):
    ...
    transferred = client.post(
        "/api/companies/1/wallets/transfer",
        headers=maker_auth,
        json={
            "source_wallet_id": 1,
...
def test_clean_company_gets_main_from_admin_then_hr_creates_payroll_wallet(
    client, app, admin_auth, maker2_auth
):
    ...
    funded = client.post(
        "/api/admin/companies/2/topup", headers=admin_auth, json={"amount": 100000}
    )
    assert funded.status_code == 200
    main_wallet = next(
        wallet
        for wallet in funded.get_json()["company"]["wallets"]
    ...
    transferred = client.post(
        "/api/companies/2/wallets/transfer",
        headers=maker2_auth,
        json={
