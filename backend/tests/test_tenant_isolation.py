...
def test_retired_direct_authorization_cannot_execute_another_tenant_batch(
    client, maker_auth, maker2_auth
):
    batch = Batch(
        company_id=2,
        maker_id=3,
    ...
    before = client.get("/api/companies/2", headers=maker2_auth).get_json()["company"][
    ...
    after = client.get("/api/companies/2", headers=maker2_auth).get_json()["company"][
        "central_wallet_balance"
    ]
