...
def test_batch_requires_its_original_maker(client, app, maker_auth):
    ...
    login = client.post(
        "/api/auth/login", json={"email": "maker2@pran.com", "password": "password123"}
    )
...
def test_checker_raises_ai_alert_as_hr_issue_before_signoff(client, maker_auth, checker_auth):
    ...
    alerts = client.get(f"/api/risk-alerts?batch_id={batch_id}", headers=checker_auth).get_json()[
        "risk_alerts"
    ]
    ai_alert = next(alert for alert in alerts if not alert["flag_type"].startswith("MANUAL_"))

    premature_signoff = client.post(
        f"/api/batches/{batch_id}/checker-review", headers=checker_auth, json={}
    )
    assert premature_signoff.status_code == 400
    assert "every ai alert" in premature_signoff.get_json()["error"].lower()

    obsolete_approval = client.put(
        f"/api/risk-alerts/{ai_alert['id']}/review",
    ...
    reviewed_alerts = client.get(
        f"/api/risk-alerts?batch_id={batch_id}", headers=checker_auth
    ).get_json()["risk_alerts"]
    reviewed_ai_alert = next(alert for alert in reviewed_alerts if alert["id"] == ai_alert["id"])
    manual_alert = next(
        alert for alert in reviewed_alerts if alert["flag_type"] == "MANUAL_INCORRECT_SALARY"
    )
    assert reviewed_ai_alert["review_status"] == "REJECTED_BY_CHECKER"
    assert manual_alert["review_status"] == "PENDING_REVIEW"

    corrected = client.put(
        f"/api/batches/items/{ai_alert['batch_item_id']}/correct",
        headers=maker_auth,
        json={
...
def test_ai_recheck_creates_a_fresh_alert_when_hr_correction_is_still_invalid(
    client, maker_auth, checker_auth
):
    response = client.post(
        "/api/batches/upload",
        headers=maker_auth,
    ...
    corrected = client.put(
        f"/api/batches/items/{item['id']}/correct",
        headers=maker_auth,
        json={
    ...
    resolved_alerts = client.get(
        f"/api/risk-alerts?batch_id={batch_id}", headers=maker_auth
    ).get_json()["risk_alerts"]
...
def test_execution_is_blocked_until_invalid_item_is_corrected(client, maker_auth, checker_auth):
    ...
    assert (
        client.put(
            f"/api/risk-alerts/{alerts[0]['id']}/review",
            headers=checker_auth,
            json={"action": "OVERRIDDEN_BY_CHECKER"},
        ).status_code
        == 200
    )
    assert (
...
def test_admin_approval_adds_employee_to_corporate_roster(client, maker_auth, admin_auth):
    ...
    pending = client.get("/api/employee-registrations", headers=admin_auth).get_json()[
        "registrations"
    ]
    registration = next(row for row in pending if row["email"] == "new.employee@example.com")
    approval = client.post(
        f"/api/employee-registrations/{registration['id']}/approve", headers=admin_auth
    )
