...
export const CheckerTab: React.FC<CheckerTabProps> = ({
  ...
  const [batchNotes, setBatchNotes] = useState(
    'All payroll items and flagged anomalies reviewed & approved for disbursement.',
  );
  ...
  const openAiAlerts = alerts.filter(
    (alert) => alert.review_status === 'PENDING_REVIEW' && !alert.flag_type.startsWith('MANUAL_'),
  );
    ...
    <div className="space-y-6">
      ...
      <div className="card-flat p-6 bg-white border border-slate-100 rounded-2xl shadow-sm relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-6 border-b border-slate-100 space-y-4 sm:space-y-0 relative z-10">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-2 max-w-xl">
              Review AI anomaly alerts, authorize high-variance transactions, and sign off batch for
              Maker final disbursement.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 pt-6 text-xs relative z-10">
          <button
            type="button"
            disabled={!currentBatch || isDownloading}
            onClick={() => void downloadSelectedBatch()}
            className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 flex items-center justify-between text-left transition hover:-translate-y-0.5 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 disabled:cursor-not-allowed disabled:opacity-60"
          >
            <div className="min-w-0 pr-3">
              ...
              <div
                className="mt-1 truncate font-outfit text-sm font-extrabold text-[#2d3142]"
                title={currentBatch?.file_name}
              >
          ...
          <button
            type="button"
            onClick={() => setActiveAlertView('ai')}
            className="bg-red-50/50 p-4 rounded-xl border border-red-100 flex items-center justify-between text-left transition hover:-translate-y-0.5 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-red-200"
          >
            <div>
              <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500">
                Open AI Risk Alerts
          ...
          <button
            type="button"
            onClick={() => setActiveAlertView('manual')}
            className="bg-orange-50/60 p-4 rounded-xl border border-orange-200 flex items-center justify-between text-left transition hover:-translate-y-0.5 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-200"
          >
            <div>
              <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500">
                Finance-raised issues
              </span>
            ...
            <div className="w-10 h-10 rounded-full bg-orange-100 text-[#ef8354] flex items-center justify-center">
          ...
          <button
            type="button"
            onClick={() => setActiveAlertView('resolved')}
            className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-100 flex items-center justify-between text-left transition hover:-translate-y-0.5 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-200"
          >
            <div>
              <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500">
                Signed-off Alerts
        ...
        {currentBatch?.status === 'PENDING_CHECKER_REVIEW' && (
          <div
            className={`mt-6 border-t border-slate-100 p-3 pt-4 relative z-10 flex flex-col items-center gap-4 rounded-xl border-dashed sm:flex-row ${hasOpenAlerts ? 'bg-amber-50/70' : 'bg-slate-50/50'}`}
          >
            <div className="flex items-center gap-2 flex-1 pl-2">
              <div
                className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-white ${hasOpenAlerts ? 'bg-amber-500' : 'bg-[#059669]'}`}
              >
                {hasOpenAlerts ? (
              ...
              {hasOpenAlerts ? (
                <div>
                  <p className="text-xs font-extrabold text-amber-900">
                    AI recheck found {openAiAlerts.length + manualAlerts.length} remaining{' '}
                    {openAiAlerts.length + manualAlerts.length === 1 ? 'issue' : 'issues'}
                  </p>
                  <p className="mt-0.5 text-[10px] text-amber-700">
                    Open the alert cards above and resolve every issue before Finance sign-off.
                  </p>
                </div>
              ) : (
                <input
            ...
            <button
              ...
              title={
                hasOpenAlerts
                  ? 'Override each valid exception or raise an issue before signing off.'
                  : undefined
              }
      ...
      <div className="card-flat overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-outfit text-base font-extrabold text-[#2d3142]">
                Payroll sheet preview
              </h3>
              <p className="mt-0.5 text-xs text-slate-500">
                {currentBatch
                  ? `${currentBatch.file_name} · ${items.length} uploaded payroll rows`
                  : 'Select a payroll batch to inspect its uploaded rows.'}
              </p>
            </div>
          </div>
          {currentBatch && (
            <div className="rounded-xl bg-slate-50 px-3 py-2 text-right">
              ...
              <p className="mt-0.5 font-mono text-sm font-extrabold text-slate-800">
                {formatAmount(currentBatch.total_amount)}
              </p>
            </div>
          )}
        </div>
        ...
        {!currentBatch ? (
          <div className="px-6 py-12 text-center text-xs text-slate-400">
            No payroll batch is available for review.
          </div>
        ) : items.length === 0 ? (
          <div className="px-6 py-12 text-center text-xs text-slate-400">
            Loading the uploaded payroll sheet…
          </div>
        ) : (
            ...
            <table className="w-full min-w-[1040px] border-collapse text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {sortedItems.map((item, index) => {
                  ...
                  const hasOpenManualIssue = manualAlerts.some(
                    (alert) => alert.batch_item_id === item.id,
                  );
                  return (
                    <tr
                      key={item.id}
                      className={hasRisk ? 'bg-amber-50/30' : 'hover:bg-slate-50/60'}
                    >
                      ...
                      <td className="px-4 py-3.5">
                        <span
                          className={`inline-flex rounded-md px-2 py-1 text-[9px] font-extrabold uppercase tracking-wide ${
                      ...
                      <td className="w-[132px] min-w-[132px] px-4 py-3.5 text-right">
                        {hasOpenManualIssue ? (
                          ...
                          <button
                            type="button"
                            disabled={
                              !['PENDING_CHECKER_REVIEW', 'RETURNED_TO_HR'].includes(
                                currentBatch.status,
                              )
                            }
      ...
      {activeSection && (
        <div
          ...
          onMouseDown={(event) => {
            if (event.currentTarget === event.target) setActiveAlertView(null);
          }}
        >
          <div
            className={`card-flat max-h-[80vh] w-full max-w-5xl overflow-auto bg-white p-6 rounded-3xl shadow-2xl border ${activeSection.accent === 'orange' ? 'border-orange-200' : activeSection.accent === 'emerald' ? 'border-emerald-200' : 'border-slate-200'}`}
          >
            <div className="flex items-center justify-between pb-4 mb-2">
              <div>
                <h3 className="font-extrabold text-[#2d3142] text-lg font-outfit">
                  {activeSection.title}
                </h3>
              ...
              <div className="flex items-center gap-3">
                <span
                  className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg border ${activeSection.accent === 'orange' ? 'border-orange-200 bg-orange-50 text-orange-700' : activeSection.accent === 'emerald' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-indigo-100 bg-indigo-50 text-indigo-800'}`}
                >
                  {activeSection.alerts.length}{' '}
                  {activeSection.key === 'manual'
                    ? 'Manual issues'
                    : activeSection.key === 'resolved'
                      ? 'Resolved'
                      : 'AI alerts'}
                </span>
                <button
                  type="button"
                  onClick={() => setActiveAlertView(null)}
                      ...
                      {activeSection.alerts.map((alert) => (
                        <tr key={alert.id} className="hover:bg-slate-50/50 transition-colors group">
                          <td className="py-4 px-2">
                            <div className="flex items-start gap-3">
                              <div
                                className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${activeSection.key === 'manual' ? 'bg-orange-50 text-[#ef8354]' : activeSection.key === 'resolved' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}
                              >
                                {activeSection.key === 'manual' ? (
                                  <Flag className="w-5 h-5" />
                                ) : activeSection.key === 'resolved' ? (
                          ...
                          <td className="py-4 px-2 text-[11px] text-slate-600 font-medium leading-relaxed pr-6">
                            {alert.anomaly_reason ||
                              `${alert.flag_type.replaceAll('_', ' ')} requires Finance Director review.`}
                          </td>
                          <td className="py-4 px-2">
                            ...
                            activeSection.key === 'ai' ? (
                              ...
                              <div className="text-[10px] font-bold text-emerald-700 bg-emerald-50 inline-block px-2.5 py-1 rounded border border-emerald-200">
                                ...
                                {alert.review_status === 'RESOLVED_BY_HR'
                                  ? `Resolved by ${alert.reviewer_name || 'HR'}`
                                  : `Reviewed by ${alert.reviewer_name || 'Finance'}`}
                              </div>
                            ) : (
                          ...
                          <td className="py-4 px-2">
                            <div className="flex items-center justify-center gap-2 whitespace-nowrap">
                              {alert.review_status === 'PENDING_REVIEW' &&
                              activeSection.key === 'ai' &&
                              ['PENDING_CHECKER_REVIEW', 'CHECKER_REVIEWED'].includes(
                                currentBatch?.status || '',
                              ) ? (
                                <>
                                  <button
                                    onClick={() =>
                                      onReviewAlert(
                                        ...
                                        selectedNotes[alert.id] ||
                                          'Exception authorized by Finance Director',
                                      )
                                    }
                                  ...
                                  <button
                                    onClick={() =>
                                      setFlaggedItem(
                                        items.find((item) => item.id === alert.batch_item_id) ||
                                          null,
                                      )
                                    }
                              ...
                              <span
                                className={`ml-2 px-2 py-1 rounded font-bold text-[10px] w-[56px] text-center ${
          ...
          >
            <div className="flex items-start justify-between border-b border-slate-100 pb-4">
              <div className="flex gap-3">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-orange-50 text-[#ef8354]">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {flaggedItem.employee_name} · {formatAmount(flaggedItem.gross_salary)}
                  </p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setFlaggedItem(null)}
            ...
            <label className="mt-5 block text-xs font-extrabold text-slate-700">
              ...
              <select
                ...
                className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-xs font-semibold text-slate-800 focus:border-[#ef8354] focus:outline-none"
              >
            ...
            <label className="mt-4 block text-xs font-extrabold text-slate-700">
              ...
              <textarea
                ...
                placeholder="Explain what looks wrong and what HR should verify…"
                className="mt-2 w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-xs leading-relaxed text-slate-800 focus:border-[#ef8354] focus:outline-none"
              />
            </label>
            <p className="mt-3 text-[11px] leading-relaxed text-slate-500">
              Returning this row pauses Finance approval. HR will see your note, correct the payroll
              row, and resubmit the batch.
            </p>
            <div className="mt-5 flex gap-3">
              <button
                type="button"
                onClick={() => setFlaggedItem(null)}
                className="flex-1 rounded-xl bg-slate-100 px-4 py-3 text-xs font-extrabold text-slate-700 hover:bg-slate-200"
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={!issueNotes.trim() || isReturning}
                className="flex-1 rounded-xl bg-[#ef8354] px-4 py-3 text-xs font-extrabold text-white shadow-sm hover:bg-[#d67045] disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isReturning ? 'Returning…' : 'Return to HR'}
              </button>
            </div>
          </form>
        </div>
