...
import { FileSpreadsheet, CloudUpload, Edit3, Send, CheckCircle2, XCircle, AlertTriangle, HelpCircle, Download, CheckCheck, LayoutGrid, Clock3, RefreshCw } from 'lucide-react';
...
export const UploadTab: React.FC<UploadTabProps> = ({
  ...
  const payrollPeriodLabel = payrollPeriod
    ? new Intl.DateTimeFormat('en', { month: 'long', year: 'numeric' }).format(new Date(`${payrollPeriod}-01T00:00:00`))
    : 'the selected month';
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end lg:gap-9">
          <div>
            ...
            <p className="text-xs text-[#4f5d75] mt-2 max-w-xl leading-relaxed">
              Upload the spreadsheet for the payroll month selected in the sidebar. That month becomes the historical period used in liquidity forecasting.
            </p>
          </div>
      ...
      <div className={`rounded-xl border p-2 shadow-sm ${currentBatch ? 'border-emerald-200 bg-emerald-50/40' : 'border-slate-100 bg-white'}`}>
            ...
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex min-w-0 items-start gap-4">
                <div className="relative grid h-14 w-14 shrink-0 place-items-center rounded-xl bg-emerald-50 text-emerald-700">
                  ...
                  <span className="absolute -bottom-1.5 -right-1.5 grid h-6 w-6 place-items-center rounded-full border-2 border-white bg-emerald-600 text-white"><CheckCircle2 className="h-3.5 w-3.5" /></span>
                </div>
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h4 className="font-outfit text-base font-extrabold text-[#2d3142]">Spreadsheet uploaded successfully</h4>
                    <span className="rounded-md bg-emerald-50 px-2 py-1 text-[9px] font-extrabold uppercase tracking-wider text-emerald-700">Uploaded</span>
                  </div>
                  <p className="mt-1.5 truncate font-mono text-xs font-bold text-[#4f5d75]">{currentBatch.file_name}</p>
                  <p className="mt-1 text-[11px] text-slate-500">{items.length} payroll rows · {payrollPeriodLabel} · Ready for validation</p>
                </div>
              </div>
              <button type="button" onClick={() => fileInputRef.current?.click()} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-[#ef8354]/60 bg-white px-5 py-3 text-xs font-extrabold text-[#ef8354] transition-all hover:bg-orange-50 active:scale-[.98] focus:outline-none focus:ring-2 focus:ring-[#ef8354]/20">
                <RefreshCw className="h-4 w-4" /><span>Replace spreadsheet</span>
              </button>
            </div>
          </div>
        ) : (
          <div onClick={() => fileInputRef.current?.click()} className="group mx-auto cursor-pointer rounded-2xl border border-dashed border-[#cbd4df] bg-[#fcfcfc] px-6 py-8 text-center transition-all hover:border-[#ef8354] hover:bg-[#ef8354]/5 sm:py-6">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-slate-100 bg-white text-[#ef8354] shadow-sm transition-transform duration-300 group-hover:scale-105"><CloudUpload className="h-8 w-8" /></div>
            <h4 className="mt-4 font-outfit text-base font-extrabold text-[#2d3142]">Drag &amp; drop your HR Payroll spreadsheet here</h4>
            <p className="mt-1.5 text-[13px] text-[#4f5d75]">Uploading for <strong className="text-[#2d3142]">{payrollPeriodLabel}</strong> · Supports .xlsx, .xls, .csv files (Up to 16MB)</p>
            <button type="button" onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click(); }} className="mt-5 inline-flex items-center space-x-2 rounded-xl bg-[#ef8354] px-7 py-3 text-sm font-bold text-white shadow-sm transition-all hover:bg-[#d67045] active:scale-[0.98]"><CloudUpload className="h-4 w-4" /><span>Browse Spreadsheet File</span></button>
          </div>
        )}
      </div>
      ...
      <div className="bg-[#ffffff] border border-slate-100 rounded-xl p-8 shadow-sm mt-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between pb-6 mb-6 space-y-4 md:space-y-0">
          <div className="flex items-start space-x-4">
            ...
            <div>
              ...
              <p className="text-[13px] text-[#4f5d75] mt-1.5">
                Multi-stage verification: account status, payroll-history coverage, and unusual salary variance checks.
              </p>
            </div>
          </div>
          ...
          {currentBatch?.status === 'PENDING_CHECKER_REVIEW' && (
            <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50/60 px-4 py-3 text-amber-900">
              <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-amber-200 bg-white text-amber-600"><Clock3 className="h-4 w-4" /></span>
              <div><p className="text-xs font-extrabold">Waiting for Finance Director review</p><p className="mt-0.5 text-[10px] text-amber-700">The submitted batch is locked until Finance completes the review.</p></div>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-[#ffffff] p-4 rounded-xl border border-slate-200 flex items-center justify-between shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            ...
            <strong className="text-[#2d3142] font-outfit text-xl font-extrabold">{limitedHistoryCount}</strong>
          </div>
          ...
          <div className="bg-[#ffffff] p-4 rounded-xl border border-slate-200 flex items-center justify-between shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)]">
            ...
            <strong className="text-[#2d3142] font-outfit text-base font-extrabold tracking-tight">BDT {totalGrossSalary.toLocaleString()}</strong>
          </div>
        </div>
              ...
              {items.length === 0 ? (
                ...
                items.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="py-4 px-2 font-extrabold text-[#2d3142] font-outfit">BDT {item.basic_salary.toLocaleString()}</td>
                    <td className="py-4 px-2 font-extrabold text-[#2d3142] font-outfit">BDT {item.gross_salary.toLocaleString()}</td>
