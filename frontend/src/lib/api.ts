...
export async function fetchBatches(companyId: number = 1) {
  const res = await apiFetch(`${API_BASE_URL}/batches?company_id=${companyId}`, {
    headers: getAuthHeaders(),
  });
...
export async function fetchBatchItems(batchId: number = 1) {
  const res = await apiFetch(`${API_BASE_URL}/batches/${batchId}/items`, {
    headers: getAuthHeaders(),
  });
...
export async function fetchEmployees(companyId: number = 1) {
  const res = await apiFetch(`${API_BASE_URL}/companies/${companyId}/employees`, {
    headers: getAuthHeaders(),
  });
...
export async function createCompanyWallet(
  ...
  const res = await apiFetch(`${API_BASE_URL}/companies/${companyId}/wallets`, {
    method: 'POST',
    headers: getAuthHeaders(),
    body: JSON.stringify({ ...payload, opening_balance: 0 }),
  });
...
export async function uploadPayrollFile(
  ...
  const res = await apiFetch(`${API_BASE_URL}/batches/upload`, {
    method: 'POST',
    body: formData,
  });
...
export async function createManualRiskAlert(batchItemId: number, issueType: string, notes: string) {
  const res = await apiFetch(`${API_BASE_URL}/risk-alerts/manual`, {
    method: 'POST',
    headers: getAuthHeaders(),
...
export async function submitBatch(batchId: number) {
  const res = await apiFetch(`${API_BASE_URL}/batches/${batchId}/submit`, {
    method: 'POST',
    headers: getAuthHeaders(),
  });
...
export async function executeBatch(batchId: number) {
  const res = await apiFetch(`${API_BASE_URL}/batches/${batchId}/execute`, {
    method: 'POST',
    headers: getAuthHeaders(),
  });
...
export async function reviewRiskAlert(alertId: number, action: string, notes: string) {
  const res = await apiFetch(`${API_BASE_URL}/risk-alerts/${alertId}/review`, {
    method: 'PUT',
    headers: getAuthHeaders(),
    body: JSON.stringify({ action, notes }),
  });
...
export async function fetchLiquidityForecast(companyId: number = 1) {
  const res = await apiFetch(`${API_BASE_URL}/analytics/liquidity-forecast/${companyId}`, {
    headers: getAuthHeaders(),
  });
...
export async function refreshLiquidityForecast(companyId: number) {
  const res = await apiFetch(`${API_BASE_URL}/analytics/liquidity-forecast/${companyId}/refresh`, {
    method: 'POST',
    headers: getAuthHeaders(),
  });
...
export async function fetchDisbursementHistory(companyId: number) {
  const res = await apiFetch(`${API_BASE_URL}/analytics/disbursement-history/${companyId}`, {
    headers: getAuthHeaders(),
  });
...
export async function fetchAuditLogs(batchId: number = 1) {
  const res = await apiFetch(`${API_BASE_URL}/audit-logs?batch_id=${batchId}`, {
    headers: getAuthHeaders(),
  });
