...
export default function Registrations() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        setCompanies((await api.getCompanies(token)).companies);
      } catch (error) {
    ...
    void (async () => {
      ...
      try {
        if (activeTab === 'approvals') {
          setRegistrations(
            (await api.getPendingEmployeeRegistrations(token, Number(companyId)))
        ...
        setMessage(error instanceof Error ? error.message : 'Unable to load company records.');
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <div className="registration-workspace">
      <section className="registration-control-card">
        <div className="registration-company-control">
          <div className="registration-control-icon">
            <Building2 size={20} />
          </div>
          <label htmlFor="registration-company">
            <span>Company</span>
            <select
            ...
            >
              <option value="">Select a company to continue</option>
        ...
        {selectedCompany && (
          <div className="registration-company-meta">
            <span>Selected company</span>
            <strong>{selectedCompany.company_name}</strong>
          </div>
        )}
      </section>
      <div className="registration-tabs" role="tablist" aria-label="Registration records">
        <button
        ...
        >
          <UserRoundCheck size={17} /> Pending approvals
        </button>
        <button
      ...
      {!companyId ? (
        <div className="registration-empty-state">
          ...
          <p>
            Choose a corporate client above to view its{' '}
            {activeTab === 'approvals'
              ? 'employee registrations awaiting approval'
              : 'registered users'}
            .
          </p>
        </div>
        ...
        <section className="registration-table-card">
          <div className="registration-table-heading">
            <div>
              <span>{activeTab === 'approvals' ? 'Approval queue' : 'User directory'}</span>
              <h3>
                {activeTab === 'approvals'
            ...
            <div className="registration-heading-actions">
              {activeTab === 'approvals' && (
                <button
                  className="registration-bulk-approve-button"
                  onClick={bulkApprove}
                ...
                >
                  {bulkApproving ? (
                    ...
                    <>
                      <CheckCheck size={16} /> Bulk approve
              ...
              <div className="registration-count">
                {activeTab === 'approvals' ? registrations.length : users.length} records
              </div>
            </div>
          </div>
          <div className="registration-table-scroll">
            {activeTab === 'approvals' ? (
              <table className="registration-table">
                <thead>
                  <tr>
                    <th className="registration-selection-cell">
                      <input
                        ref={selectAllRef}
                        type="checkbox"
                        checked={allSelected}
                        ...
                        disabled={!registrations.length || bulkApproving}
                      />
                    </th>
                    <th>Employee</th>
                    <th>Employee code</th>
                ...
                <tbody>
                  {loading ? (
                    <tr>
                      <td colSpan={7} className="registration-table-empty">
                    ...
                    registrations.map((row) => (
                      <tr key={row.id} className={selectedIds.includes(row.id) ? 'selected' : ''}>
                        <td className="registration-selection-cell">
                          <input
                            type="checkbox"
                            checked={selectedIds.includes(row.id)}
                            onChange={() => toggleSelection(row.id)}
                            aria-label={`Select ${row.full_name}`}
                            disabled={bulkApproving || approvingId === row.id}
                          />
                        </td>
                        <td>
                        ...
                        <td className="registration-number">{row.wallet_details}</td>
                        <td>
                        ...
                        <td>
                          <button
                            ...
                            disabled={approvingId === row.id || bulkApproving}
                          >
                            {approvingId === row.id ? (
                              'Approving…'
                            ) : (
                    ...
                    <tr>
                      <td colSpan={7} className="registration-table-empty">
                        No employee registrations are waiting for approval.
                      </td>
                    </tr>
                  )}
                </tbody>
              ...
              <table className="registration-table">
                ...
                <tbody>
                  {loading ? (
                    <tr>
                      <td colSpan={4} className="registration-table-empty">
                        Loading users…
                      </td>
                    </tr>
                  ) : users.length ? (
                    users.map((user) => (
                      <tr key={user.id}>
                        ...
                        <td className="registration-number">{user.phone_number}</td>
                        <td>
                          <span className="registration-role">{user.role}</span>
                        </td>
                        <td>
                          <span className={`registration-status ${user.status.toLowerCase()}`}>
                            {user.status}
                          </span>
                        </td>
                      </tr>
                    ...
                    <tr>
                      <td colSpan={4} className="registration-table-empty">
                        No registered users or approved employees were found for this company.
                      </td>
                    </tr>
                  )}
                </tbody>
